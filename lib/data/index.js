...
function connectionInfo() {
	...
	db.infoAsync('server').then(server => logger.debug('data.server', {server}));
	db.infoAsync('keyspace').then(keys => logger.debug('data.keys', {keys}));
...
function keepalive(poll) {
	if (host) {
		...
		logger.error('process.env.GURUMOJO_REDIS_HOSTNAME required', omit(poll, 'interval'));
		clearInterval(poll.interval);
	}
}
